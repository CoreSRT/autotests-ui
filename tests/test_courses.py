...
def test_empty_courses_list():
    with sync_playwright() as playwright:
        browser = playwright.chromium.launch(
            headless=False
            # везде в тестах убрал slow_mo, так как он теперь слишком замедляет прогоны
        )
        context = browser.new_context()
        page = context.new_page()

        # Использую noqa, чтобы PyCharm не ругался на дублирующийся код
        page.goto('https://nikita-filonov.github.io/qa-automation-engineer-ui-course/#/auth/registration')  # noqa
    ...
    with sync_playwright() as playwright:  # noqa
        browser = playwright.chromium.launch(
            headless=False
        )
